...
var myMap = L.map("map", {
    ...
    zoom: 3,
    layers: [graymap, SlopeOfferingVariety],
...
d3.json("https://raw.githubusercontent.com/elu267/Group_Project_Unit18/master/Resources/skiResorts_geojson.json", function(err, data) {
    ...
    function getColor(SlopeOfferingVariety) {
        return SlopeOfferingVariety > 5 ? '#8F3B1B' :
            SlopeOfferingVariety > 4.9 ? '#D57500' :
            SlopeOfferingVariety > 3.9 ? '#404F24' :
            SlopeOfferingVariety > 2.9 ? '#668D3C' :
            SlopeOfferingVariety > 1.9 ? '#DBCA69' :
            SlopeOfferingVariety > .9 ? '#4E6172' :
            '#816C5B';
    }

    function style(feature) {
        return {
            fillColor: getColor(feature.properties.SlopeOfferingVariety),
            opacity: 1,
            fillOpacity: 1,
            color: 'black',
    ...
    L.geoJson(data, {
        ...
        }).addTo(SlopeOfferingVariety),

        SlopeOfferingVariety.addTo(myMap);
    // });
    ...
    legend.onAdd = function() {
        ...
        div.innerHTML += '<b>Slope Offering Rating</b><br>'
